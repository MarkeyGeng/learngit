...
def train():
    dataFile = AAG('age', dataSetDir)     #加载数据集
    ...
    for epoch in range(nepoch):
        for img, age in dataLoader:
            img, age = Variable(img), Variable(age)
            out = model(img)
            loss = criterion(out, age.squeeze())
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
            cnt += 1
    ...
    torch.save(model.state_dict(), '{0}/model2.pth'.format(model_cp))
